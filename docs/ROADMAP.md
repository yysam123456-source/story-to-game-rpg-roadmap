...
| 10 | 微信小程序并行集成 | 抽出共享剧情 core，建立小程序原生播放器 | P1 |
...
让同一份 Story-to-Game JSON 同时运行在 HTML 播放器和微信小程序播放器中。HTML 继续承担开源、离线和独立部署能力，小程序承担中文用户移动端传播、微信分享和未来商业化入口。
...
- 抽出 `story-engine`、`condition-engine`、`change-engine`、`save-model`。
- HTML 播放器改为调用共享 core。
- 建立微信小程序最小播放器。
- 小程序端加载内置修仙 Demo JSON。
- 小程序端支持状态栏、选项、变化反馈、条件置灰和本地存档。
- Q4 接入云端作品列表、微信分享、`interactions` 和 `inventory`。
...
小程序在 MVP 阶段只作为技术验证并行推进：抽出共享 core，验证同一份修仙 Demo JSON 能在小程序端播放，不进入 Q3 产品主线范围。
...
- 构建时打包单文件。
- 发布物仍保持“一个 HTML 可运行”。
- 共享剧情 core 后，HTML 和小程序复用同一套剧情推进、条件判断和状态变化逻辑。
