# Story-to-Game v4.0 平台版产品需求文档（PRD）

**日期**：2026-07-06
**版本**：v4.0（平台版）
**状态**：草案 -> 待评审
**基于**：PRD v3.0 + 架构升级方向（静态前端 + 轻量 API + JSON 文件存储）
...
| 维度 | Story-to-Game v4.0 | Ink/Inky | Twine | AI Dungeon | 橙光/易次元 |
|------|---------------------|----------|-------|------------|-------------|
...
3. **静态前端 + 轻量 API + JSON 文件存储能承载完整产品体验**，无需传统数据库和用户系统
...
### 3.1 整体架构：静态前端 + 轻量 API + JSON 文件存储
...
静态前端（rpg-game-ui-v2/）
  |-- index.html          # 首页/引流入口
  |-- create.html         # 创作页（粘贴/上传小说）
  |-- play.html           # 播放器页（RPGStoryLoader 加载 JSON）
  |-- gallery.html        # 作品库页（浏览 + 点击即玩）
  |-- assets/             # CSS/JS/图片等静态资源
  |
  v
轻量 API 服务（Python Flask / Node Express）
  |-- POST /api/generate  # 接收小说文本 + 类型参数，AI 生成 JSON，保存到 files/
  |-- GET  /api/works     # 列出 files/ 目录下所有作品
  |-- GET  /api/works/:id # 返回指定作品的 JSON 文件内容
  |
  v
JSON 文件存储（服务端 files/ 目录）
...
7. 后端调用 AI 生成 JSON，保存到 files/
8. 返回作品 ID
9. 前端跳转到 play.html?id={uuid}
10. 播放器通过 GET /api/works/{uuid} 获取 JSON
11. RPGStoryLoader 加载 JSON，用户即刻游玩
```
...
4. 用户点击某个作品
5. 跳转到 play.html?id={uuid}
6. 播放器通过 GET /api/works/{uuid} 获取 JSON
7. RPGStoryLoader 加载，用户开始游玩
```
...
**功能**：接收小说文本和生成参数，调用 AI 生成 RPG JSON，保存到 `files/` 目录。
...
3. 调用 AI API（OpenAI / Claude / 本地模型）
4. 解析 AI 返回的 JSON，运行 `validate.py` 校验
5. 校验通过：保存到 `files/{uuid}.json`，返回作品 ID
...
**后端行为**：
1. 根据短 ID 匹配 `files/` 目录下的 JSON 文件
2. 读取并返回完整 JSON 内容
3. 文件不存在时返回 404
...
```
Layer 0: 平台服务（v4.0 新增）
  - 轻量 API 服务（POST /api/generate, GET /api/works, GET /api/works/:id）
  - JSON 文件存储（服务端 files/ 目录）
  - AI Prompt 构造与调用

Layer 1: 叙事引擎（已有，rpg-game-ui-v2/ 播放器）
  - 节点/段落/选项/分支/结局
  - 状态变量/标记/成就
  - RPGStoryLoader 加载 JSON 游玩
...
**需求描述**：搭建平台版核心后端服务，提供 AI 生成和作品管理能力。

**技术选型**：Python Flask 或 Node Express（二选一，根据团队技术栈决定）。

**服务规格**：
- 仅 3 个接口：`POST /api/generate`、`GET /api/works`、`GET /api/works/:id`
- 无数据库，JSON 文件保存在服务端 `files/` 目录
- 无用户系统，无登录/注册
- AI 调用封装为独立模块，支持切换不同 AI Provider
- 内置 `validate.py` 校验，生成的 JSON 必须通过 Schema 校验

**验收标准**：
- 3 个接口均可正常调用
- AI 生成的 JSON 保存到 `files/` 目录
- 生成的 JSON 通过 `validate.py` 校验
- 服务启动和部署简单（单进程即可）
...
**主题色映射**：`var(--brand-color)` 和 `var(--brand-glow)` 随 `meta.genre` 自动切换。
...
**需求描述**：在播放器（rpg-game-ui-v2/）顶部增加 RPG 状态栏，展示 `primaryStats`。播放器统一指向 `rpg-game-ui-v2/`，通过 `RPGStoryLoader` 加载 JSON。
...
**技术实现**：
- `RPGStoryLoader` 从 API 或本地加载 JSON
...
6. 生成完成后自动跳转到 `play.html?id={uuid}`
7. 播放器加载 JSON，用户即刻游玩
...
**需求描述**：播放器（rpg-game-ui-v2/）支持从 API 加载 JSON，而非仅内嵌本地 JSON。

**实现方式**：
- 播放器 URL 格式：`play.html?id={uuid}`
- 页面加载时，`RPGStoryLoader` 调用 `GET /api/works/{uuid}` 获取 JSON
- 获取成功后加载游玩
...
| 编号 | 需求 | 说明 |
|------|------|------|
| P2-1 | JSON 驱动 UI 对接 | 将 UI V2 的 Glassmorphism 组件与 JSON 引擎对接，实现真实数据驱动的渲染 |
...
**播放器架构**：沿用现有 `rpg-game-ui-v2/` 的 HTML/CSS/JS 架构，不做 React/Vue 重构。播放器统一指向 `rpg-game-ui-v2/`，通过 `RPGStoryLoader` 加载 JSON 游玩。
...
**框架**：Python Flask 或 Node Express（二选一）

**存储**：JSON 文件存储在服务端 `files/` 目录，不使用数据库

**用户系统**：不做用户注册/登录，不做后台管理

**接口数量**：仅 3 个接口（POST /api/generate, GET /api/works, GET /api/works/:id）

**部署**：单进程即可满足初期需求，支持 Docker 容器化部署
...
rpg-game-ui-v2/
  ...
  |   |   |-- apocalypse.css
  |   |   |-- palace.css
  |   |-- main.css        # 基础样式（含 Glassmorphism）
  |   |-- rpg.css         # RPG 状态栏样式
  |   |-- milestone.css   # 里程碑庆祝覆盖层样式
  |   |-- ending.css      # 结局 UI / dot tracker 样式
  |   |-- page.css        # 平台页面布局样式（v4.0 新增）
  |   +-- mobile.css      # 移动端适配
  |-- js/
  |   |-- RPGStoryLoader.js  # JSON 加载器（本地 + API）
  |   |-- engine.js       # 核心引擎（节点推进）
  |   |-- state.js        # 状态管理
  |   |-- rpg.js          # RPG 系统
  |   |-- delayed-change.js  # 延迟变化系统
  |   |-- milestone.js    # 里程碑系统
  |   |-- ending.js       # 结局系统
  |   |-- render.js       # 渲染
  |   |-- vfx.js          # VFX 特效引擎
  |   |-- storage.js      # 存档/读档
  |   |-- devtools.js     # 调试面板
  |   |-- api.js          # API 调用封装（v4.0 新增）
  |   |-- page.js         # 平台页面交互逻辑（v4.0 新增）
  |   +-- validation.js   # 运行时校验
  +-- assets/             # 图片等静态资源
```
...
```
server/
  |-- app.py               # Flask/Express 入口（3 个路由）
  |-- ai/
  |   |-- generator.py     # AI 调用封装
  |   |-- prompts/
  |   |   |-- base.txt     # 基础 Prompt 模板
  |   |   |-- xianxia.txt  # 修仙类型 Prompt
  |   |   |-- horror.txt   # 恐怖类型 Prompt
  |   |   |-- mystery.txt  # 悬疑类型 Prompt
  |   |   |-- apocalypse.txt  # 末世类型 Prompt
  |   |   +-- palace.txt   # 宫斗类型 Prompt
  |   +-- provider.py      # AI Provider 切换（OpenAI/Claude/本地）
  |-- validate.py         # JSON 校验器
  |-- files/               # 生成的 JSON 文件存储目录
  +-- config.py            # 配置（AI API Key 等）
```
...
v4.0 的 UI 视觉标准沿用 UI V2 原型。以下为已实现的设计系统完整规范。
...
#### 7.0.2 Design Tokens 体系（`tokens.css`）
...
- **不做后台管理系统**：files/ 目录即存储，无需 CMS
- **不做数据库**：JSON 文件存储，简单可靠
...
- **不做 React/Vue 重构**：沿用现有 HTML/CSS/JS 架构
...
| RPGStoryLoader | 播放器的 JSON 加载器，支持本地和 API 加载 |
