...
以下需求按优先级排序，P0-1 到 P0-5 为核心功能，P0-6 为 Demo 内容。
...
检查 pending delayedChanges
  ↓
如果有，按添加顺序依次：
  1. 插入 reason 文本到叙事流（作为新段落，不阻断选择）
  2. 应用 changes 数值变化
  3. 如果 changes.show === true，显示变化反馈 Toast
  ↓
继续正常叙事（渲染 triggerNode 的 segments）
```

**reason 文本处理**：reason 字段的内容已在 JSON 中写好（不是动态生成），保证叙事质量。reason 以旁白风格插入到 triggerNode 的 segments 之前，例如：

```
师父放下茶盏，沉默了片刻。
[reason插入] 你忽然想起，三个月前你隐瞒了在后山发现的那本秘籍...
师父看了你一眼，目光中闪过一丝失望。
→ 师父信任 -10
```

**多个 delayedChanges 同时触发**：按添加顺序依次处理，每个 delayedChanges 独立显示 reason 和变化反馈。
...
当 `changes.apply()` 后同时满足 milestones 和 endings 条件时：

**检测顺序**：
1. 先检测 milestones（成长节点优先展示，给玩家成就感）
2. 再检测 endings（如果当前节点是 candidateEndings 节点）

**同时触发时的展示规则**：
- milestone 和 ending 同时满足时，先展示 milestone 庆祝动画
- 如果 ending 实际触发（玩家到达结局节点），milestone 的 celebration 简化为不阻断的简短通知（避免阻碍结局动画）
- milestone 的触发不影响 ending 的判定逻辑
...
**需求描述**：建立最小微信小程序播放器，验证同一份修仙 Demo JSON 能在微信环境中稳定播放。该任务不替代 HTML 主线，可作为技术预研并行推进，后续进入内测版。
...
| 编号 | 需求 | 说明 |
|------|------|------|
| P2-1 | JSON 驱动 UI 对接 | 将 UI V2 的 Glassmorphism 组件（scene-viewport、status-bar、narrative、interactions、inventory、notifications、achievements、endings）与 JSON 引擎对接，实现真实数据驱动的渲染 |
| P2-2 | 悬疑/末世/宫斗模板 | 三类各一个 Demo，包含 milestones、endings、delayedChanges |
| P2-3 | 示例库与文档 | 每类至少一个完整 Demo + 教程 |
| P2-4 | 微信小程序内测版 | 作品列表、云端 JSON 加载、分享卡片、云存档评估 |
| P2-5 | 云存储 + 多设备同步 | 可选功能 |
| P2-6 | 创作者平台 | 上传/分享/收费；小程序可作为优先分发入口 |
...
### 5.2 扩展布局（含 interactions + 深度探索 + 完整组件）
...
- **不优先实现完整题材模板**：5 种题材主题色/VFX 在 UI V2 中已有视觉原型，但对应的 JSON 内容模板（完整的 milestones/endings/interactions 定义）放在 P1/P2
