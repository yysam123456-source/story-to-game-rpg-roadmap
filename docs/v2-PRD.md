...
在节点（node）级别新增 `delayedChanges` 字段：
...
```json
{
  ...
  "delayedChanges": {
    ...
    "changes": {
      ...
      "show": true,
      "narrativeHint": "你守了一整夜的丹炉，虽然灵力有所消耗，但师父看你的眼神中多了一丝赞许。"
    }
  }
}
```
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `target` | string | 否 | 延迟目标，`"next"` 表示下一个节点进入时生效（默认值）。未来可扩展为指定节点 ID |
| `changes` | object | 是 | 与标准 `changes` 结构一致 |
| `changes.show` | boolean | 否 | 是否在目标节点中显示反馈 |
| `changes.narrativeHint` | string | 否 | 融入目标节点叙事流的提示文本，由播放器插入到目标节点 segments 之后 |
...
玩家在节点 A 做出选择:
  ...
  2. 将 A 的 delayedChanges 存入延迟队列
  3. 进入节点 B 时:
     a. 先渲染 B 的 segments
     b. 执行延迟队列中所有 delayedChanges
     c. 如果 delayedChanges.narrativeHint 存在，追加到 B 的叙事文本末尾
     d. 如果 delayedChanges.show === true，显示 Toast
     e. 清空已执行的 delayedChanges
```

**验收标准**：
- `delayedChanges` 在下一节点进入时正确生效
- `narrativeHint` 文本自然融入当前节点叙事
- 第一层（changes.show）和第二层（delayedChanges）可以同时存在，互不干扰
- 第三层后果通过 flags/routes 系统正常工作
- `delayedChanges` 未设置时完全不影响旧 JSON

**工作量**：5 天（延迟队列 + 执行引擎 + narrativeHint 渲染）
...
| `infinite-horror` | 生存闪光 | 线索发现（恐惧闪屏） | 存活归来（画面撕裂重组） |
| `mystery` | 线索微闪 | 推理成功（打字机效果） | 案件破解（线索聚合爆发） |
| `post-apocalypse` | 资源提示 | 避难所升级（警告闪光） | 幸存者集结（噪点闪烁 + 光芒） |
| `palace-intrigue` | 金色微光 | 权力晋升（帷幕微动） | 登顶（帷幕拉开 + 金色粒子） |
...
| `choice.weightLabel` | `choices[].weightLabel` | 使用 weight 对应的默认标签文本 | 可选覆盖默认标签 |
...
| `delayedChanges.changes.narrativeHint` | `delayedChanges.changes.narrativeHint` | 不追加任何叙事文本 | 可选字段，提供时不影响 segments 渲染 |
...
| 无限恐怖 | `infinite-horror` | 深红 `#8B1A1A` | 血橙 `#D4644A` | 深黑 `#0D0A0A` | 灰白 `#C8C0B8` |
| 悬疑 | `mystery` | 冷蓝 `#3A5F8A` | 银灰 `#8A9FB5` | 深蓝黑 `#0A0F1A` | 淡蓝 `#C8D5E8` |
| 末日 | `post-apocalypse` | 军绿 `#5A6B3C` | 铁锈 `#8B7355` | 深绿黑 `#0D110A` | 暗绿白 `#C8CCB8` |
| 宫斗 | `palace-intrigue` | 金粉 `#C8906C` | 绛紫 `#8B5A6B` | 深酒红 `#1A0D10` | 暖粉白 `#E8D5CC` |
...
| 延迟变化（delayedChanges） | 节点级字段，定义延迟到下一个节点生效的变化 |
| 叙事提示（narrativeHint） | `delayedChanges` 中的可选字段，融入目标节点叙事流的文本 |
