# Story-to-Game JSON Schema v1.0

**日期**：2026-07-04  
**状态**：🔒 已锁定（Phase 1 交付物）  
**覆盖范围**：文学模式（baseline）+ RPG 扩展（meta.rpg / milestones / endings / delayedChanges / interactions / inventory）
...
| `candidateEndings` | array\<string\> | ❌ | 声明该节点为结局候选节点，见 §9.3 |
...
| `condition` | string/object | ❌ | 执行条件，见 §10 |
...
| `condition` | string/object | ✅ | 触发条件，见 §10 |
...
| `condition` | string/object | ✅ | 触发条件，见 §10.2 |
...
> **Schema 锁定声明**：本文档（v1.0）已锁定，后续修改需经阶段一评审流程。新字段提案在 `docs/SCHEMA_v1.x_PROPOSAL.md` 中记录，评审通过后并入下一版。
